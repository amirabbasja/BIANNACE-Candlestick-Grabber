...
if UPDATE:
    ...
    for file in files:
        z = re.match(f"HistoricalCandles_{SYMBOL}_{TIMEFRAME}_(\d+)_(\d+).xlsx", file)
    ...
    if len(_dict) != 0:
        ...
        candles.to_excel(f"./data/HistoricalCandles_{SYMBOL}_{TIMEFRAME}_{int(candles.iloc[0,0])}_{int(candles.iloc[-1,0])}.xlsx", index = False)
